%%% Run 1st level analysis for all models together for Feedback Appraisal Study - Keller data
...
data_folder='D:\Feedback_reward\Data\Data_Keller2021';
...
subject_folder(1:3)=[]; % CHANGE THIS LINE!!
first_subj=1;
...
%     %% Concatenate realignment parameters
    ...
    for sess = 1:n_sess
        ...
        for run = 1:n_runs
            ...
            rpFolder=[subjFolder filesep 'D' num2str(sess) filesep 'NF' num2str(run)];
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_spec_model1_keller.mat'])
    ...
    if length(num2str(subj))==1
        ...
        load([subjFolder filesep 'Cond_files' filesep 'conds_subj' num2str(subj) '.mat']) % conditions specs
    end
    ...
    for sess=1:n_sess
        ...
        for run=1:nruns
            ...
            func_source=[subjFolder filesep 'D' num2str(sess) filesep 'NF' num2str(run)]; % functional images folder
            ...
            func_cell_temp{1,1}(1:10)=[]; % first 10 volumes are treated as dummy scans
    ...
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(3).onset = vec_emotion_onset_all;
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(3).duration = vec_emotion_duration_all; 
...
%     spm_jobman('run',matlabbatch);
    ...
    scans=[ones(1,n_sess*nruns)*(len-10)];
...
%     spm_fmri_concatenate(spmFile1, scans);
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_estim_model1_keller.mat'])
...
%     spm_jobman('run',matlabbatch);
    ...
    load([analysisFolder 'batch_createFcontrast_model1_keller.mat'])
...
%     extract_anatomical_rois(cfg)
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_spec_model2_keller.mat'])
    ...
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(4).onset = vec_emotion_onset_all;
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(4).duration = vec_emotion_duration_all; 
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(5).onset = vec_percent_onset_all;
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(5).duration = vec_percent_duration_all; 
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(6).onset = vec_training_onset_all;
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(6).duration = vec_training_duration_all; 
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(7).onset = vec_view_onset_all;
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.cond(7).duration = vec_view_duration_all; 
...
%     spm_fmri_concatenate(spmFile2, scans);
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_estim_model2_keller.mat'])
...
%     spm_jobman('run',matlabbatch);
    ...
    load([analysisFolder 'batch_createFcontrast_model2_keller.mat'])
...
%     extract_anatomical_rois(cfg)
    ...
    load([analysisFolder 'batch_createPPI_pmod-base_model3_keller.mat'])
...
%     spm_jobman('run',matlabbatch);
    ...
    load([analysisFolder 'batch_GLM_PPI_FB_pmod_model3_keller.mat'])
...
%     spm_jobman('run',matlabbatch);
    ...
    load([analysisFolder 'batch_createPPI_pmod-base_model4_keller.mat'])
    ...
    load([analysisFolder 'batch_GLM_PPI_FB_pmod_model4_keller.mat'])
