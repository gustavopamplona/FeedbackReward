% Define parameters for 1st Level analysis - Feedback Reward - Scheinost 2020
...
data_file=[analysisFolder filesep 'Dist_Scheinost2020\fbvalues_scheinost.xlsx'];
data_mat=table2cell(readtable(data_file));

onset_file=[analysisFolder filesep 'Scheinost2020_task-nfb_events.csv'];
onset_table=readtable(onset_file);
onset_cell=table2cell(onset_table);

FBonset_file=[analysisFolder filesep 'feedback_onset_times.xlsx'];
        ...
        for run=1:n_runs
            ...
            if sess == 1
                ...
                if length(num2str(run+5))==1
                    funcFolder=[data_folder filesep subject_folder(subj).name '\func\Run' num2str(run+5)];
%                 else
...
%                         vec_reg_onset(n)=onset_cell{i,1}; % Pamplona
...
%                     if strcmp(onset_cell(i,3),'feedback') && strcmp(mat_censor(cell2mat(onset_cell(i,4)),run),'no') && cell2mat(onset_cell(i,4))<=n_valid_trials % only gets the uncensored trials %Pamplona
                    if strcmp(onset_cell(i,3),'feedback') && strcmp(mat_censor(cell2mat(onset_cell(i,4)),run),'no') && cell2mat(onset_cell(i,4))<n_valid_trials % only gets the uncensored trials %Scheinost
%                         vec_reg2_onset(o)=onset_cell{i+2,1}; % Pamplona
                        ...
                        o=o+1;
                    elseif strcmp(onset_cell(i,3),'regulation')
%                         vec_reg2after_onset(q)=onset_cell{i,1}+4; % rest of the regulation block % Pamplona
...
%             cumul_time=[cumul_time TR*nVols]; % Pamplona
