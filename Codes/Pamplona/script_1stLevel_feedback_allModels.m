%%% Run 1st level analysis for all models together for Feedback Appraisal Study - Pamplona data

% Gustavo Pamplona, 20.01.2022
...
%         subjFolder=[data_folder filesep 'A0' num2str(subj)];
%     else
%         subjFolder=[data_folder filesep 'A' num2str(subj)];
%     end
%     
%     cumul_c=[];
%     for run = 1:nruns
...
%     dlmwrite([subjFolder filesep filename '.txt'],cumul_c,'delimiter','\t','precision','%.10f')
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_spec_model1.mat'])
    
    if length(num2str(subj))==1
        directory1=[folderName '\Data\Data_Pamplona\A0' num2str(subj) '\1stLevel_1']; % directory
        subj_folder=[folderName '\Data\Data_Pamplona\A0' num2str(subj)]; % subject folder
        load([subj_folder filesep 'conds_subj0' num2str(subj) '.mat']) 
        multi_reg_file=[subj_folder '\mult_reg_subj0' num2str(subj) '.txt']; % multiple regressors file
    else
        directory1=[folderName '\Data\Data_Pamplona\A' num2str(subj) '\1stLevel_1']; % directory
        subj_folder=[folderName '\Data\Data_Pamplona\A' num2str(subj)]; % subject folder
        load([subj_folder filesep 'conds_subj' num2str(subj) '.mat']) 
    ...
    for run=1:nruns
        if length(num2str(run))==1
            func_source=[subj_folder '\Run0' num2str(run)]; % functional images folder
        else
            func_source=[subj_folder '\Run' num2str(run)]; % functional images folder
        end
        func_path{1,:}=func_source;
        ...
        for j=1:length(funcFiles)
            if length(funcFiles(j).name)>2
                if strcmp(funcFiles(j).name(1,:),['swraFunc.nii'])
    ...
    scans=ones(1,nruns)*len;

    subj
    
    if length(num2str(subj))==1
        spmFile1=[folderName '\Data\Data_Pamplona\A0' num2str(subj) '\1stLevel_1\SPM.mat'];
    else
        spmFile1=[folderName '\Data\Data_Pamplona\A' num2str(subj) '\1stLevel_1\SPM.mat'];
    end
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_estim_model1.mat'])
    ...
    load([analysisFolder 'batch_createFcontrast_model1.mat'])
    ...
    if length(num2str(subj))==1
        cfg.name=['NAcc_A0' num2str(subj)];
    else
        cfg.name=['NAcc_A' num2str(subj)];
    end
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_spec_model2.mat'])
    
    if length(num2str(subj))==1
        directory2=[folderName '\Data\Data_Pamplona\A0' num2str(subj) '\1stLevel_2']; % directory
    else
        directory2=[folderName '\Data\Data_Pamplona\A' num2str(subj) '\1stLevel_2']; % directory
    end
    
    matlabbatch{1,1}.spm.stats.fmri_spec.dir{1,1}=directory2;
    matlabbatch{1,1}.spm.stats.fmri_spec.sess.scans=func_cell{1,1};
    ...
    if length(num2str(subj))==1
        spmFile2=[folderName '\Data\Data_Pamplona\A0' num2str(subj) '\1stLevel_2\SPM.mat'];
    else
        spmFile2=[folderName '\Data\Data_Pamplona\A' num2str(subj) '\1stLevel_2\SPM.mat'];
    end
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_estim_model2.mat'])
    ...
    load([analysisFolder 'batch_createFcontrast_model2.mat'])
    ...
    if length(num2str(subj))==1
        cfg.name=['dlPFC_A0' num2str(subj)];
    else
        cfg.name=['dlPFC_A' num2str(subj)];
    end
    ...
    load([analysisFolder 'batch_createPPI_pmod-base_model3.mat'])
    
    if length(num2str(subj))==1
        VOIfile1=[directory1 '\VOI_NAcc_A0' num2str(subj) '_1.mat'];
        PPIname1=['PPI_sub0' num2str(subj) '_FB_model3_Nacc'];
    else
        VOIfile1=[directory1 '\VOI_NAcc_A' num2str(subj) '_1.mat'];
    ...
    load([analysisFolder 'batch_GLM_PPI_FB_pmod_model3.mat'])
    ...
    load([analysisFolder 'batch_createPPI_pmod-base_model4.mat'])
    
    if length(num2str(subj))==1
        VOIfile2=[directory2 '\VOI_dlPFC_A0' num2str(subj) '_1.mat'];
        PPIname2=['PPI_sub0' num2str(subj) '_FB_model4_dlPFC'];
    else
        VOIfile2=[directory2 '\VOI_dlPFC_A' num2str(subj) '_1.mat'];
    ...
    load([analysisFolder 'batch_GLM_PPI_FB_pmod_model4.mat'])
    ...
    clear PPI xY matlabbatch func_cell func_cell_temp
