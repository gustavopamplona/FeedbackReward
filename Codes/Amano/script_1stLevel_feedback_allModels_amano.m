%%% Run 1st level analysis for all models together for Feedback Appraisal Study - Amano data

% Gustavo Pamplona, 28.07.2022
...
for subj=first_subj:nruns
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_spec_model1_amano.mat'])
    
    if length(num2str(subj))==1
        directory1=[folderName '\Data\Data_Amano\sub-0' num2str(subj) '\1stLevel_1']; % directory
        subj_folder=[folderName '\Data\Data_Amano\sub-0' num2str(subj)]; % subject folder
        ...
        directory1=[folderName '\Data\Data_Amano\sub-' num2str(subj) '\1stLevel_1']; % directory
        subj_folder=[folderName '\Data\Data_Amano\sub-' num2str(subj)]; % subject folder
...
%         count runs
        S = dir([subj_folder '\func\ses-' num2str(sess)]);
        ...
        for run=1:n_runs
            ...
            if length(num2str(subj))==1
                func_source=[subj_folder '\func\ses-' num2str(sess) '\sub-0' num2str(subj) '_task-nfb_bold_run-' num2str(run) '.nii']; % functional images folder
            else
                func_source=[subj_folder '\func\ses-' num2str(sess) '\sub-' num2str(subj) '_task-nfb_bold_run-' num2str(run) '.nii']; % functional images folder
            end
            func_path{1,:}=func_source;
            ...
            for j=1:length(funcFiles)
                if length(funcFiles(j).name)>2
                    if strcmp(funcFiles(j).name(1,:),['swraday' num2str(sess) '-run' num2str(run) '.nii'])
    ...
    if length(num2str(subj))==1
        spmFile1=[folderName '\Data\Data_Amano\sub-0' num2str(subj) '\1stLevel_1\SPM.mat'];
    else
        spmFile1=[folderName '\Data\Data_Amano\sub-' num2str(subj) '\1stLevel_1\SPM.mat'];
    end
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_estim_model1_amano.mat'])
    ...
    load([analysisFolder 'batch_createFcontrast_model1_amano.mat'])
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_spec_model2_amano.mat'])
    
    if length(num2str(subj))==1
        directory2=[folderName '\Data\Data_Amano\sub-0' num2str(subj) '\1stLevel_2']; % directory
    else
        directory2=[folderName '\Data\Data_Amano\sub-' num2str(subj) '\1stLevel_2']; % directory
    end
    ...
    load([analysisFolder 'batch_1stLevel_Feedback_estim_model2_amano.mat'])
    ...
    load([analysisFolder 'batch_createFcontrast_model2_amano.mat'])
    ...
    load([analysisFolder 'batch_createPPI_pmod-base_model3_amano.mat'])
    ...
    load([analysisFolder 'batch_GLM_PPI_FB_pmod_model3_amano.mat'])
    ...
    load([analysisFolder 'batch_createPPI_pmod-base_model4_amano.mat'])
    ...
    load([analysisFolder 'batch_GLM_PPI_FB_pmod_model4_amano.mat'])
